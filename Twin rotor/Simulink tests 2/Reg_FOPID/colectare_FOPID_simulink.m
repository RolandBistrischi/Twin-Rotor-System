%Date din simulink pt FOPID
close all;clear all; clc;
azimuth=load("D:\MATLAB\Licenta_fractionar\Twin-Rotor-System\Twin rotor\Simulink tests\Reg_FOPID\date_step_azimuth.mat");
pitch=load("D:\MATLAB\Licenta_fractionar\Twin-Rotor-System\Twin rotor\Simulink tests\Reg_FOPID\date_step_pitch.mat");
...
folderPaths = {
    'D:\MATLAB\Licenta_fractionar\Twin-Rotor-System\Twin rotor\date_regulatoare\Reg_FOPID\H11_nou\', ...
    'D:\MATLAB\Licenta_fractionar\Twin-Rotor-System\Twin rotor\date_regulatoare\Reg_FOPID\H22_nou\', ...
...
Reg_FOPID_H11 = [
    loadedData(1).RegFOPID_H11_ISE,
    %loadedData(1).RegFOPID_H11_ITSE,
    %loadedData(1).RegFOPID_H11_IAE,
    loadedData(1).RegFOPID_H11_ITAE,
    %loadedData(1).RegPID_indian_H11_sensibility
    ];

Reg_FOPID_H22 = [
    loadedData(2).RegFOPID_H22_ISE, 
    %loadedData(2).RegFOPID_H22_ITSE, 
    %loadedData(2).RegFOPID_H22_IAE, 
    loadedData(2).RegFOPID_H22_ITAE, 
...
for i = 1:length(Reg_FOPID_H11)
    Gc =minreal(oustapp( Reg_FOPID_H11(i).regulator));
    bode(Gc * H11);
end
...
figure;
for i = 1:length(Reg_FOPID_H11)
    a = Reg_FOPID_H11(i).iteration;  % Accesează numărul de iterații pentru acest regulator
    iteration = 1:a;  % Vectorul de iterații

    % Accesează vectorul 'best' pentru fiecare regulator
    best_values = Reg_FOPID_H11(i).best(init:end);
...
for i = 1:length(Reg_FOPID_H22)
    Gc =minreal(oustapp( Reg_FOPID_H22(i).regulator));
    bode(Gc * H22);
end
...
figure;
for i = 1:length(Reg_FOPID_H22)
    a = Reg_FOPID_H22(i).iteration;  % Accesează numărul de iterații pentru acest regulator
    iteration = 1:a;  % Vectorul de iterații

    % Accesează vectorul 'best' pentru fiecare regulator
    best_values = Reg_FOPID_H22(i).best(init:end);
    % Asigură-te că lungimea lui 'iteration' și 'best_values' sunt egale
    if length(iteration) ~= length(best_values)
        iteration = iteration(1:length(best_values));  % Ajustează lungimea dacă e nevoie
    end
